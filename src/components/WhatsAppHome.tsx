...
export default function WhatsAppHome() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (selectedChat.type === 'individual') {
      ...
      messageContent = currentInput; // Don't add character name for user messages
    ...
    try {
      ...
      const apiMessageContent = selectedChat.type === 'group' && targetCharacterId
        ? `@${targetCharacterId} ${messageContent}`
        : selectedChat.type === 'individual'
          ? `@${selectedChat.id} ${messageContent}`
          : messageContent;
